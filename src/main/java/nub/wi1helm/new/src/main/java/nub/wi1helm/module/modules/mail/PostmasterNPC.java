package nub.wi1helm.module.modules.mail;

import net.kyori.adventure.sound.Sound;
import net.kyori.adventure.text.Component;
import net.kyori.adventure.text.minimessage.MiniMessage;
import net.minestom.server.coordinate.Pos;
import net.minestom.server.entity.Entity;
import net.minestom.server.entity.Player;
import net.minestom.server.entity.PlayerHand;
import net.minestom.server.entity.PlayerSkin;
import net.minestom.server.event.player.PlayerEntityInteractEvent;
import net.minestom.server.instance.Instance;
import net.minestom.server.item.ItemStack;
import net.minestom.server.sound.SoundEvent;
import net.minestom.server.tag.Tag;
import nub.wi1helm.Utils;
import nub.wi1helm.entity.EntityManager;
import nub.wi1helm.entity.npc.NPC;
import nub.wi1helm.goals.GoalManager;
import nub.wi1helm.goals.ServerGoals;
import nub.wi1helm.item.ItemManager;
import org.jetbrains.annotations.NotNull;

import java.util.Arrays;
import java.util.List;
import java.util.UUID;

public class PostmasterNPC extends NPC {

    private static final PlayerSkin POSTMASTE_SKIN = new PlayerSkin(
            "",
            "");

    private final List<Mailbox> mailboxes;
    private final GoalManager goalManager;
    private final ItemManager itemManager;

    public PostmasterNPC(List<Mailbox> mailboxes, GoalManager goalManager, ItemManager itemManager) {
        super(POSTMASTE_SKIN, Component.text("Postmaster Matt"));

        this.mailboxes = mailboxes;
        this.goalManager = goalManager;
        this.itemManager = itemManager;
    }

    @Override
    public void spawn(@NotNull EntityManager entityManager, @NotNull Instance instance, @NotNull Pos position) {
        super.spawn(entityManager, instance, position);
        enableLookAtPlayers(getMinestomEntities(), position.yaw(), position.pitch(), 5F);
    }

    @Override
    public void spawn(EntityManager manager, Instance instance) {

    }

    @Override
    public void onPlayerInteract(PlayerEntityInteractEvent event) {
        final Player player = event.getPlayer();
        final Entity npc = event.getTarget();

        if (event.getHand() != PlayerHand.MAIN) return;

        if (!goalManager.hasActiveGoal(ServerGoals.DELIVER_MAIL)) {
            player.sendActionBar(MiniMessage.miniMessage().deserialize("<aqua>Sorry no packages for you today</aqua>"));
            player.playSound(Sound.sound().type(SoundEvent.ENTITY_VILLAGER_NO).source(Sound.Source.NEUTRAL).build());
            return;
        };

        if (goalManager.isGoalCompleted(ServerGoals.DELIVER_MAIL)) {
            player.sendActionBar(MiniMessage.miniMessage().deserialize("<white>Thank you for helping. We done here</white>"));
            player.playSound(Sound.sound().type(SoundEvent.ENTITY_VILLAGER_TRADE).pitch(1.3F).source(Sound.Source.NEUTRAL).build());
            return;
        }

        long num = Arrays.stream(event.getPlayer().getInventory().getItemStacks())
                .filter(item -> item.hasTag(PackageItem.ITEM_TAG))
                .mapToLong(ItemStack::amount) // sum the number of items in the stack
                .sum();


        if (num >= 3) {
            player.sendActionBar(MiniMessage.miniMessage().deserialize("<red>You already have some packages man!!</red>"));
            player.playSound(Sound.sound().type(SoundEvent.ENTITY_VILLAGER_NO).pitch(0.4F).source(Sound.Source.NEUTRAL).build());
            return;
        }

        Mailbox mailbox = Utils.randomElement(mailboxes);

        PackageItem item = new PackageItem(mailbox.getAddress(), mailbox.getUUID());

        event.getPlayer().getInventory().addItemStack(item.getPlayerItem(itemManager));

        player.sendActionBar(MiniMessage.miniMessage().deserialize("<green>Deliver this to the right address!</green>"));
        player.playSound(Sound.sound().type(SoundEvent.ENTITY_VILLAGER_YES).source(Sound.Source.NEUTRAL).build());


    }

    @Override
    public @NotNull Tag<@NotNull String> getEntityTag() {
        return Tag.String("module:mail:npc:postmaster");
    }


}
